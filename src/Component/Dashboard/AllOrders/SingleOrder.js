...
const SingleOrder = ({ order, refetch, i, currentPage, postsPerPage }) => {
  const confirming = () => {
    axios
      .put(`https://quickinun.com/server/order/${order._id}`, { confirm: true })
      .then(({ data }) => {
  ...
  const deleting = () => {
    ...
    if (confirmed) {
      axios
        .delete(`https://quickinun.com/server/cancel-order/${order._id}`)
        .then(({ data }) => {
          if (data?.acknowledged) {
  ...
  const holding = () => {
    axios
      .put(`https://quickinun.com/server/order/${order._id}`, { hold: true })
      .then(({ data }) => {
        if (data?.acknowledged) {
